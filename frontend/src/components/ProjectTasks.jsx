...
function ProjectTasks() {
  ...
  const fetchTasks = async () => {
    ...
    let url = `https://jira-clone-mtig.onrender.com/items/projects/${selectedProject.id}/items`;
  ...
  const fetchUsers = async () => {
    ...
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/projects/${selectedProject.id}/members`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
  ...
  const fetchColumns = async () => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/projects/${selectedProject.id}/columns`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
  ...
  const fetchEpics = async () => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/items/projects/${selectedProject.id}/items`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
    if (res.ok) {
      ...
      setEpics(
        (data.items || []).filter(
          (i) => i.type && i.type.toLowerCase() === "epic"
        )
      );
    }
  };
  ...
  const handleEditSave = async () => {
    ...
    await fetch(`https://jira-clone-mtig.onrender.com/items/${editTaskId}`, {
      method: "PATCH",
      headers: {
  ...
  const handleDelete = async (id) => {
    ...
    Modal.confirm({
      ...
      onOk: async () => {
        ...
        try {
          const res = await fetch(
            `https://jira-clone-mtig.onrender.com/items/${id}`,
            {
              method: "DELETE",
              headers: { Authorization: `Bearer ${token}` },
            }
          ...
          if (res.ok) {
            fetchTasks().catch((e) =>
              console.error("fetchTasks error after delete:", e)
            );
  ...
  const handleBugSubmit = async (values) => {
    ...
    if (columns && columns.length > 0) {
      const todoCol = columns.find(
        (col) =>
          (col.status || col.name.toLowerCase().replace(/\s/g, "")) === "todo"
      );
    ...
    if (
      payload.due_date &&
      typeof payload.due_date === "object" &&
      payload.due_date.format
    ) {
  ...
  // Group tasks by epic
  const epicsList = tasks.filter(
    (t) => t.type && t.type.toLowerCase() === "epic"
  );
  const tasksByEpic = {};
  ...
  epicsList.forEach((epic) => {
    ...
    if (expandedEpics.includes(epic.id)) {
      tasksByEpic[epic.id].forEach((child) =>
        groupedTasks.push({ ...child, isChild: true, parentEpicId: epic.id })
      );
    }
  });
  ...
  const taskTableColumns = [
    ...
    },
    {
      ...
      render: (text, record) => {
        ...
        if (record.isChild)
          return (
            <span style={{ marginLeft: 32, display: "inline-block" }}>
              {text}
            </span>
          );
        return (
  ...
  const renderChildTasks = (children) => (
    <AntList
      ...
      renderItem={(child) => (
        ...
        >
          <div style={{ display: "flex", alignItems: "center" }}>
            <ArrowRightOutlined
              style={{ color: "#722ed1", marginRight: 8, fontSize: 16 }}
            />
            ...
            {child.status && (
              <Tag style={{ marginLeft: 12 }}>{child.status}</Tag>
            )}
            {child.priority && (
              <Tag
                color={
      ...
      locale={{
        emptyText: <span style={{ color: "#bbb" }}>No child tasks</span>,
      }}
    />
  );
  ...
  return (
    ...
    >
      ...
      <Table
        ...
        bordered
        rowClassName={(record) =>
          record.isEpic ? "epic-row" : record.isChild ? "child-task-row" : ""
        }
        style={{ marginTop: 16 }}
      />
      <Modal
        ...
        <Form form={taskForm} layout="vertical" onFinish={handleTaskSubmit}>
          ...
          >
            <Select
              ...
              onChange={(val) => {
                setTaskType(val);
                taskForm.setFieldsValue({ parent_id: undefined });
              }}
            >
          ...
          <Form.Item label="Assignee" name="assignee_id">
            ...
            >
              {user.id && (
                <Option value={user.id} key="me">
                  Assign to me ({user.username || user.email || "Me"})
                </Option>
              )}
              {users
