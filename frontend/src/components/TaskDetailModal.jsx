...
export default function TaskDetailModal({ isOpen, onRequestClose, taskId }) {
  const { hasPermission, currentUser, projectMembers } =
  ...
  const fetchTask = async () => {
    ...
    try {
      const taskRes = await fetch(
        `https://jira-clone-mtig.onrender.com/items/${taskId}`,
        {
  ...
  const handleSave = async () => {
    try {
      ...
      await fetch(`https://jira-clone-mtig.onrender.com/items/${taskId}`, {
        method: "PATCH",
        headers: {
  ...
  const handleAddComment = async () => {
    ...
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/items/${task.id}/comments`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
  ...
  const canEdit =
    hasPermission("edit_any_task") ||
    (hasPermission("edit_own_task") &&
      task &&
      (currentUser?.id === task.reporter_id ||
        currentUser?.id === task.assignee_id));

  const assigneeOptionList = projectMembers.map((m) => ({
    value: m.user_id,
    label: m.username || m.email || m.user_id,
  }));
  console.log("Assignee dropdown options:", assigneeOptionList);
  const assigneeDisplay =
    task &&
    (task.assignee_name ||
      projectMembers.find((m) => m.user_id === task.assignee_id)?.username ||
      projectMembers.find((m) => m.user_id === task.assignee_id)?.email ||
      "Unassigned");
