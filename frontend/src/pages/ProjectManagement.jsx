...
function ProjectManagement() {
  ...
  const fetchProjects = async () => {
    ...
    try {
      const res = await fetch("https://jira-clone-mtig.onrender.com/projects", {
  ...
  const handleDelete = async (projectId) => {
    ...
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/projects/${projectId}`,
      {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      }
    ...
    if (res.ok) {
      ...
      const data = await res.json();
      message.error(
        data.error || "Failed to delete project. You may not have permission."
      );
    }
  };
  ...
  const fetchMembers = async (projectId) => {
    ...
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/projects/${projectId}/members`,
      {
        headers: { Authorization: `Bearer ${token}` },
      }
    );
  ...
  const handleAddMember = async (values) => {
    ...
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/projects/${memberProject.id}/members`,
      {
        method: "POST",
        headers: {
    ...
    if (res.ok) {
      ...
      setError(
        data.error || "Failed to add member. You may not have permission."
      );
    }
  };
  ...
  const handleRemoveMember = async (userId) => {
    ...
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/projects/${memberProject.id}/members/${userId}`,
      {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      }
    ...
    if (res.ok) {
      ...
      setError(
        data.error || "Failed to remove member. You may not have permission."
      );
    }
  };
  ...
  const projectColumns = [
    ...
    { title: "Description", dataIndex: "description", key: "description" },
    {
      ...
      render: (_, record) => (
        <Space>
          <Button onClick={() => openMemberModal(record)}>
            Manage Members
          </Button>
          <Popconfirm
  ...
  return (
    <div style={{ padding: 32 }}>
      ...
      <Modal
        ...
        title={
          memberProject
            ? `Manage Members: ${memberProject.name}`
            : "Manage Members"
        }
        footer={null}
        width={520}
