...
function Teams() {
  ...
  const fetchMyTeams = async () => {
    ...
    try {
      ...
      const res = await fetch(
        "https://jira-clone-mtig.onrender.com/teams/my-teams",
        {
  ...
  const handleCreateTeam = async (values) => {
    const token = localStorage.getItem("token");
    try {
      const res = await fetch("https://jira-clone-mtig.onrender.com/teams", {
        method: "POST",
        headers: {
  ...
  const handleViewTeam = async (team) => {
    ...
    try {
      const res = await fetch(
        `https://jira-clone-mtig.onrender.com/teams/${team.id}`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleAddMember = async (values) => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/teams/${selectedTeam.id}/members`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (res.ok) {
      ...
      message.error(
        data.error || "Action failed. Only team admins can add members."
      );
    }
  };
  ...
  const handleRemoveMember = async (userId) => {
    const token = localStorage.getItem("token");
    const res = await fetch(
      `https://jira-clone-mtig.onrender.com/teams/${selectedTeam.id}/members/${userId}`,
      {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      }
    ...
    if (res.ok) {
      ...
      message.error(
        data.error || "Action failed. Only team admins can remove members."
      );
    }
  };
  ...
  const columns = [
    ...
    },
    {
      ...
      key: "actions",
      render: (_, record) => (
        <Button type="primary" ghost onClick={() => handleViewTeam(record)}>
          View Details
        </Button>
      ),
    },
    ...
    <div style={{ padding: 32 }}>
      ...
      >
        {teamDetail ? (
          <div>
            <p>
              <b>Description:</b> {teamDetail.description || "N/A"}
            </p>
