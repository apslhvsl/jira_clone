    ...
    const fetchReport = async () => {
      ...
      try {
        const res = await fetch(
          `https://jira-clone-mtig.onrender.com/reports/project/${projectId}`,
          {
  ...
  if (myProjectRole !== "admin" && myProjectRole !== "manager") {
    return (
      <div style={{ padding: 32, color: "#faad14", fontWeight: 500 }}>
        ...
        <br />
        Your role: <Tag>{myProjectRole}</Tag>
      </div>
    );
  }
    ...
    <Card style={{ maxWidth: 1000, margin: "32px auto", minHeight: 600 }}>
      ...
      <Row gutter={24} style={{ marginBottom: 32 }}>
        <Col xs={24} sm={12} md={6}>
          <Statistic title="Total Tasks" value={report.stats.total} />
        </Col>
        <Col xs={24} sm={12} md={6}>
          <Statistic
            title="Done"
            value={report.stats.done}
      ...
      <List
        ...
        renderItem={(m) => (
          <List.Item>
            <span style={{ fontWeight: 500 }}>{m.username || m.email}</span>{" "}
            <Tag>{m.role}</Tag>
          </List.Item>
        )}
        bordered
